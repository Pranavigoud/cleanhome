...
export default function FrequencyModal({ isOpen, onClose, onBack, onContinue, onShowBedrooms, currentStep = 2, totalSteps = 11 }) {
  const [selectedFrequency, setSelectedFrequency] = useState('');
  const [otherText, setOtherText] = useState('');

  const frequencies = [
    { id: 'daily', label: 'Daily' },
    { id: 'twice-week', label: 'Twice a week' },
  ...
  const handleContinue = () => {
    if (selectedFrequency) {
      onShowBedrooms({ frequency: selectedFrequency, other: otherText });
    }
  };
