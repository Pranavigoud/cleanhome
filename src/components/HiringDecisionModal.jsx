...
export default function HiringDecisionModal({ isOpen, onClose, onBack, onShowLocation, currentStep = 10, totalSteps = 11 }) {
  const [selectedOption, setSelectedOption] = useState('');
  const [otherText, setOtherText] = useState('');

  const options = [
    { id: 'ready-now', label: "I'm ready to hire now" },
    { id: 'definitely-hire', label: "I'm definitely going to hire someone" },
    { id: 'likely-hire', label: "I'm likely to hire someone" },
    { id: 'possibly-hire', label: "I will possibly hire someone" },
    { id: 'planning-researching', label: "I'm planning and researching" },
    { id: 'other', label: 'Other' },
  ];

  const handleContinue = () => {
    if (selectedOption) {
      onShowLocation({ hiringDecision: selectedOption, other: otherText });
    }
  };
