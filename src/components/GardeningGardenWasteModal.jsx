...
export default function GardeningGardenWasteModal({ isOpen, onClose, onNext, onBack, progress = 0 }) {
  const [selectedWaste, setSelectedWaste] = useState('');
  const [otherValue, setOtherValue] = useState('');

  const wasteOptions = [
    { id: 'can-care', label: 'I can take care of the garden waste' },
    { id: 'pro-remove', label: 'I would like the pro to remove the garden waste' },
    { id: 'no-waste', label: 'There will be no garden waste' },
    { id: 'discuss', label: 'I need to discuss with the pro' },
    { id: 'other', label: 'Other' },
  ];

  const handleContinue = () => {
    if (selectedWaste && onNext) {
      const value = selectedWaste === 'other' ? otherValue : selectedWaste;
      onNext(value);
      setSelectedWaste('');
      setOtherValue('');
    }
      ...
      <div className="bg-white rounded-2xl max-w-sm w-full max-h-[80vh] overflow-hidden relative">
        ...
        <div className="p-3 sm:p-4">
          ...
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-500">Step 7 of 10</p>
          </div>
            ...
            {wasteOptions.map((option) => (
              ...
              >
                <input
                  ...
                  name="waste"
                  value={option.id}
                  checked={selectedWaste === option.id}
          ...
          <div className="flex justify-between gap-3">
            ...
            <button
              ...
              disabled={!selectedWaste || (selectedWaste === 'other' && !otherValue.trim())}
              className="px-6 py-2.5 rounded-lg font-semibold text-white text-sm transition-all"
              style={{
                backgroundColor: (selectedWaste && (selectedWaste !== 'other' || otherValue.trim())) ? '#0D6B7D' : '#D1D5DB',
                cursor: (selectedWaste && (selectedWaste !== 'other' || otherValue.trim())) ? 'pointer' : 'not-allowed',
              }}
            >
              Continue
            </button>
