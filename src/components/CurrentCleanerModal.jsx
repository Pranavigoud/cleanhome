...
export default function CurrentCleanerModal({ isOpen, onClose, onBack, onShowBestDays, currentStep = 7, totalSteps = 11 }) {
  const [selectedOption, setSelectedOption] = useState('');
  const [otherText, setOtherText] = useState('');

  const options = [
    { id: 'yes-replacing', label: 'Yes, replacing a current cleaner' },
    { id: 'no-previously', label: 'No, previously had a cleaner' },
    { id: 'no-never', label: 'No, never had a cleaner' },
    { id: 'other', label: 'Other' },
  ];

  const handleContinue = () => {
    if (selectedOption) {
      onShowBestDays({ currentCleaner: selectedOption, other: otherText });
    }
  };
