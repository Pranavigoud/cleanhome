...
export default function BathroomsModal({ isOpen, onClose, onBack, onShowReceptionRooms, currentStep = 4, totalSteps = 11 }) {
  const [selectedBathrooms, setSelectedBathrooms] = useState('');
  const [otherText, setOtherText] = useState('');

  const bathrooms = [
    { id: '1', label: '1 bathroom' },
    { id: '1-toilet', label: '1 bathroom + 1 additional toilet' },
    { id: '2', label: '2 bathrooms' },
    { id: '2-toilet', label: '2 bathrooms + 1 additional toilet' },
    { id: '3', label: '3 bathrooms' },
    { id: '4plus', label: '4+ bathrooms' },
    { id: 'other', label: 'Other' },
  ];

  const handleContinue = () => {
    if (selectedBathrooms) {
      onShowReceptionRooms({ bathrooms: selectedBathrooms, other: otherText });
    }
  };
          ...
          <div className="space-y-2 mb-6">
            {bathrooms.map((bathroom) => (
              <div
                ...
                onClick={() => setSelectedBathrooms(bathroom.id)}
              >
                <input
                  type="radio"
                  name="bathrooms"
                  value={bathroom.id}
                  checked={selectedBathrooms === bathroom.id}
