...
export default function GardeningHiringDecisionModal({ isOpen, onClose, onNext, onBack, progress = 0 }) {
  const [selectedDecision, setSelectedDecision] = useState('');
  const [otherValue, setOtherValue] = useState('');

  const decisionOptions = [
    { id: 'ready-now', label: "I'm ready to hire now" },
    { id: 'definitely', label: "I'm definitely going to hire someone" },
    { id: 'likely', label: "I'm likely to hire someone" },
    { id: 'possibly', label: 'I will possibly hire someone' },
    { id: 'planning', label: "I'm planning and researching" },
    { id: 'other', label: 'Other' },
  ];

  const handleContinue = () => {
    if (selectedDecision && onNext) {
      const value = selectedDecision === 'other' ? otherValue : selectedDecision;
      onNext(value);
      setSelectedDecision('');
      setOtherValue('');
    }
      ...
      <div className="bg-white rounded-2xl max-w-sm w-full max-h-[80vh] overflow-hidden relative">
        ...
        <div className="p-3 sm:p-4">
          ...
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-500">Step 9 of 10</p>
          </div>
          ...
          {selectedDecision === 'other' && (
            <div className="mb-4 p-2.5 bg-gray-50 rounded-lg border border-gray-300">
              ...
              <input
                ...
                placeholder="Enter your hiring likelihood"
          ...
          <div className="flex justify-between gap-3">
            ...
            <button
              ...
              disabled={!selectedDecision || (selectedDecision === 'other' && !otherValue.trim())}
              className="px-6 py-2.5 rounded-lg font-semibold text-white text-sm transition-all"
              style={{
                backgroundColor: (selectedDecision && (selectedDecision !== 'other' || otherValue.trim())) ? '#0D6B7D' : '#D1D5DB',
                cursor: (selectedDecision && (selectedDecision !== 'other' || otherValue.trim())) ? 'pointer' : 'not-allowed',
              }}
            >
              Continue
            </button>
