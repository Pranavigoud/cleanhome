...
export default function GardeningGardenConditionModal({ isOpen, onClose, onNext, onBack, progress = 0 }) {
  const [selectedCondition, setSelectedCondition] = useState('');
  const [otherValue, setOtherValue] = useState('');

  const conditionOptions = [
    { id: 'overgrown', label: 'Overgrown' },
    { id: 'well-kept', label: 'Well-kept' },
    { id: 'in-between', label: 'Somewhere in between' },
    { id: 'other', label: 'Other' },
  ];

  const handleContinue = () => {
    if (selectedCondition && onNext) {
      const value = selectedCondition === 'other' ? otherValue : selectedCondition;
      onNext(value);
      ...
      <div className="bg-white rounded-2xl max-w-sm w-full max-h-[80vh] overflow-hidden relative">
        ...
        <div className="p-3 sm:p-4">
          ...
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-500">Step 5 of 10</p>
          </div>

          {/* Title */}
          <h2 className="text-lg sm:text-xl font-bold text-center text-gray-900 mb-4">
            What's the current condition of the garden?
          </h2>
          ...
          {selectedCondition === 'other' && (
            <div className="mb-4 p-2.5 bg-gray-50 rounded-lg border border-gray-300">
              ...
              <input
                ...
                placeholder="Enter garden condition"
          ...
          <div className="flex justify-between gap-3">
            ...
            <button
              ...
              disabled={!selectedCondition || (selectedCondition === 'other' && !otherValue.trim())}
              className="px-6 py-2.5 rounded-lg font-semibold text-white text-sm transition-all"
              style={{
                backgroundColor: (selectedCondition && (selectedCondition !== 'other' || otherValue.trim())) ? '#0D6B7D' : '#D1D5DB',
                cursor: (selectedCondition && (selectedCondition !== 'other' || otherValue.trim())) ? 'pointer' : 'not-allowed',
              }}
            >
              Continue
            </button>
